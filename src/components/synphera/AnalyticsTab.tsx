...
import { getAssets, getROIFacts, getDepartmentROIMatrix, getTotalEnterpriseValue, getVersionSnapshots, type DbPromptAsset, type DbROIFact } from '@/lib/supabase-store';
import { DEPARTMENTS, ROI_CATEGORIES, Department, ROICategory } from '@/lib/synphera-types';
import { ResponsiveContainer, PieChart, Pie, Cell, Tooltip } from 'recharts';
import { DollarSign, TrendingUp, GitBranch, Building2, Shield, Activity } from 'lucide-react';
...
export function AnalyticsTab({ refreshKey }: AnalyticsTabProps) {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const [a, f, tv, rm, vs] = await Promise.all([
        getAssets(), getROIFacts(), getTotalEnterpriseValue(), getDepartmentROIMatrix(), getVersionSnapshots()
      ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="metric-card">
          <CardHeader className="pb-2">
            <CardDescription className="flex items-center gap-2 text-xs"><Activity className="h-3.5 w-3.5" />Version Snapshots</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-2xl lg:text-3xl font-bold text-foreground font-mono">{versionCount}</p>
            <p className="text-xs text-muted-foreground mt-1">Avg {(versionCount / Math.max(assets.length, 1)).toFixed(1)} per asset</p>
          </CardContent>
        </Card>
        <Card className="metric-card">
          <CardHeader className="pb-2">
            <CardDescription className="flex items-center gap-2 text-xs"><Building2 className="h-3.5 w-3.5" />Departments</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-2xl lg:text-3xl font-bold text-foreground font-mono">{new Set(assets.map(a => a.department)).size}</p>
            <p className="text-xs text-muted-foreground mt-1">of {DEPARTMENTS.length} total</p>
          </CardContent>
        </Card>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            {assets.length > 0 ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-12">No assets yet. Create some to see the lineage chart.</p>
            )}
          </CardContent>
        </Card>
